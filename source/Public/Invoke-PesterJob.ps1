<#
    .SYNOPSIS
        Runs Pester tests using a runspace-based approach.

    .DESCRIPTION
        The `Invoke-PesterJob` command runs Pester tests using a runspace-based approach.
        ...
        Its primary purpose is to run Pester tests in a separate runspace to avoid polluting
...
function Invoke-PesterJob
{
    ...
    [Diagnostics.CodeAnalysis.SuppressMessageAttribute('PSUseUsingScopeModifierInNewRunspaces', '', Justification = 'This is a false positive. The script block is used in a runspace and does not use variables from the parent scope, they are passed as parameters.')]
    ...
    # Create runspace pool for executing Pester tests
    ...
    try
    {
        ...
        $scriptBlock = {
            ...
            Write-Information -MessageData 'Running build task ''noop'' inside the runspace to setup the test pipeline.' -InformationAction 'Continue'
            ...
            {
                $pesterObject = Invoke-Pester @PesterConfiguration
            }
            else
            {
        ...
        # Add the script block and parameters to the PowerShell instance
        $null = $powerShell.AddScript($scriptBlock)
        $null = $powerShell.AddParameters(@{
                PesterConfiguration     = $pesterConfig
                ShowError               = $ShowError.IsPresent
                PesterVersion           = $importedPesterModule.Version
                BuildScriptPath         = $BuildScriptPath
                BuildScriptParameter    = $BuildScriptParameter
                EnableSourceLineMapping = $EnableSourceLineMapping
            })

        # BUG: Using $powerShell.Invoke() causes the same issue as with StartJob (CommandsMissed is not return with the object)
        # Execute and wait for completion
        # BUG: WORKS (no output): Invoke-PesterJob -Path 'tests/Unit/Pri*' -EnableSourceLineMapping -Output 'None'
        # BUG: FAILS (any output): Invoke-PesterJob -Path 'tests/Unit/Pri*' -EnableSourceLineMapping -Output 'Minimal'
        $asyncResult = $powerShell.BeginInvoke()
        $pesterResult = $powerShell.EndInvoke($asyncResult)
        ...
        if ($powerShell.HadErrors)
        {
            ...
            {
                Write-Error -ErrorRecord $streamError
            }
        }
    }
    finally
